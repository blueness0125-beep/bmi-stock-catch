/**
 * 시장 국면 데이터 서버 전용 모듈.
 * Server Component 및 API Route에서 직접 호출 (HTTP 자기 요청 없음).
 */

import type { MarketGateResponse } from "./api"
...
export async function getMarketGate(): Promise<MarketGateResponse | null> {
  try {
